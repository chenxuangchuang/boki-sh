package com.chenxc.cxcboki.message.service.impl;
...
public class MessageServiceImpl implements MessageService {
    ...
    public SendMessageResDTO sendMessage(SendMessageReqDTO sendMessageReqDTO) {
        SendMessageResDTO sendMessageResDTO = new SendMessageResDTO();
        try{
            sendMessageReqDTO.setMoblie("+86"+sendMessageReqDTO.getMoblie());
            String s = sendMessageByTencentClound(sendMessageReqDTO);
            sendMessageResDTO.setResult(s);
        } catch (TencentCloudSDKException e) {
            e.printStackTrace();
            logger.error("调用TencentCloudSDK异常",e);
        } catch (Exception e){
            e.printStackTrace();
            logger.error("调用sendMessage异常",e);
        }
        return sendMessageResDTO;
    }

    public String sendMessageByTencentClound(SendMessageReqDTO sendMessageReqDTO) throws TencentCloudSDKException {
        /* 必要步骤：
        ...
        String sessionContext = sendMessageReqDTO.getUserName()+":"+sendMessageReqDTO.getMoblie();
        ...
        String templateId = "1263207";
        req.setTemplateId(templateId);

        /* 下发手机号码，采用 E.164 标准，+[国家或地区码][手机号]
         * 示例如：+8613711112222， 其中前面有一个+号 ，86为国家码，13711112222为手机号，最多不要超过200个手机号 */
        String[] phoneNumberSet = {sendMessageReqDTO.getMoblie()};
        req.setPhoneNumberSet(phoneNumberSet);

        /* 模板参数: 若无模板参数，则设置为空 */
        String[] templateParamSet = {"1015"};
